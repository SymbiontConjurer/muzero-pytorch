...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--case",
        required=True,
        choices=["atari", "classic_control", "box2d"],
    ...
    if args.case == "atari":
        ...
        from config.classic_control import (
            muzero_config,
        )  # just using same config as classic_control for now
    ...
    try:
        if args.opr == "train":
            ...
            if args.use_wandb:
                wandb.init(
                    project="muzero-pytorch",
                    sync_tensorboard=True,
                    config=muzero_config.get_hparams(),
                )
            ...
            ), "model not found at {}".format(muzero_config.model_path)
            model = muzero_config.get_uniform_network().to("cpu")
            model.load_state_dict(
                torch.load(muzero_config.model_path, map_location=torch.device("cpu"))
            )
            test_score = test(
                muzero_config,
                model,
